...
void memoria_select(char *tabla, uint16_t key) {
	...
	if(segmento) {
		...
		if(pagina) {
			log_info(logger, "Existe la página | value: %s", obtener_value_de_pagina(pagina->ptr_posicion));
		}
		else {
			log_debug(logger, "Solicitar key a lissandra");
			t_est_tdp *frame_libre = obtener_frame_libre();
			if(!frame_libre) {
				//Acá es necesario hacer un journal
			} else {
				log_debug(logger, "Hay un frame libre");
			}
		}
	}
	else {
